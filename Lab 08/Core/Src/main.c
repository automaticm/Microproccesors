...
int main(void)
{
  ...
  int digit = 0;
  GPIOD->ODR = 1 << digit;
  Seven_Segment(0x00000000);
  while (1)
  {
	 int value = GPIOC->IDR >> 12;

	 // Sets the output of GPIOD-ODR (LEDS) to only the
	 // last 4 LEDs on to the corresponding switch SW[15:12]
	 GPIOD->ODR = (GPIOD->ODR & ~(0b1111 << 12)) | (value << 12);

	 // Checks to see if PC[10] is on or pressed.
	 // If it is, set the LED[digit] on whilst others are off.
	 if(!(GPIOC->IDR & (1 << 10))){
		  digit++;
		  digit %= 8;
		  GPIOD->ODR = 1 << digit;
	 ...
	 // Checks to see if PC[11] is on or pressed.
	 // If it is, call Sevent_Segment_Digit()
	 // which sets the [digit] seven segment display
	 // to a hex value, value..
	 if(!(GPIOC->IDR & (1 << 11))){
		  Seven_Segment_Digit(digit, value);
		  HAL_Delay(500);
	 }
    /* USER CODE BEGIN 3 */
  }
...
void Seven_Segment_Digit (unsigned char digit, unsigned char hex_char)
{
	...
	*******************************************************************************/
		...
		//GPIOE->ODR = 0xFF00;
		GPIOE->ODR = ((GPIOE->ODR & 0xFF00) | (_7SEG[hex_char])) & ~(1 << (8 + digit));
		//GPIOE->ODR &= ~(1 << (8+ digit));
		GPIOE->ODR |= 0xFF00;
		return;
}
...
*******************************************************************************/
	...
	for (digit=0 ; digit<8 ; digit++)
	{
		...
		//
	   Seven_Segment_Digit(digit, (HexValue >> (digit<<2) & 0b1111));  //0xF
	}
