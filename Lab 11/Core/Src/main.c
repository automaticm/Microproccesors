...
int CRC_Tx = 0x54E57C3A;
int CRC_Rx = 0;

/* GOODBYE ECE-330L */
char Message[] =
		{SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,
		CHAR_G,CHAR_O,CHAR_O,CHAR_D,CHAR_B,CHAR_Y,CHAR_E,SPACE,
		SPACE,CHAR_E,CHAR_C,CHAR_E,DASH,CHAR_3,CHAR_3,CHAR_0,CHAR_L,
		SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE};
...
int main(void)
{
  ...
  *******************************************************************************************************/
  ...
  while (1)
  {
	  ...
	  //********* Reset CRC value ********************
	  RCC->AHB1ENR |= (1<<12); // Enabling the clock
	  CRC->CR |= 1; // Then we reset the data register.

	  // Calculating CRC for all bytes of "message"
	  for(int i = 0; i < Message_Length; i++){
		  // Put the bytes into CRC
		  CRC->DR = Message[i];
	  }
